...
A short overview of Foundry’s Application class can be seen here.
...
Since these functions are not Foundry specific (and indeed mirror the ones for basic Arrays in javascript) only a few Foundry specific examples will be presented here while the following links should be referred to for explanation on the find and filter functions.
...
If you can’t remember how map and filter worked check out the relevant documentation on them online.
...
const steve=Actor.create({ //
type:"npc",
name:"Steve"
})

steve.createEmbeddedDocuments("Item",[{
name:"Dagger",
type:"weapon"
}]
)
